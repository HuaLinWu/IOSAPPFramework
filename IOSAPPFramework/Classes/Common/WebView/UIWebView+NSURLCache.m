...
- (void)downLoadWithRequest:(NSURLRequest *)request
{
    ...
    NSURLSessionDataTask *dataTask = [[NSURLSession sharedSession] dataTaskWithRequest:tempRequset completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        ...
        {
          //加载失败时候
          NSCachedURLResponse *cachedURLResponse = [self cachedURLResponseForRequest:tempRequset];
         [self loadData:cachedURLResponse.data response:cachedURLResponse.response request:tempRequset];
        }
        else{
            NSHTTPURLResponse *tempResponse = (NSHTTPURLResponse *)response;
            if(tempResponse.statusCode==200){
                //表示200成功
                NSURLCacheStoragePolicy cacheStoragePolicy = [self cacheStoragePolicyWithResponse:response];
            if(cacheStoragePolicy == NSURLCacheStorageAllowed){
              //表示需要缓存
              [self storeCacheForRequest:tempRequset response:response data:data];
              }
            [self loadData:data response:response request:tempRequset];
        }
